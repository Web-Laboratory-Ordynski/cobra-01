...
function ifCircleInscribed(lengthOfCircle, perimeterOfSquere) {
    var result = "------введите длину окружности и периметр квадрата------";
      if (isNaN(Number(lengthOfCircle)) || isNaN(Number(perimeterOfSquere))) {
        result = "------НЕ ЧИСЛО :(------";
      } else {
        var raduiusOfCircle = lengthOfCircle/(2*3.14);
        var squereSide = perimeterOfSquere/4;
          if (raduiusOfCircle === squereSide/2){
    ...
    var perimeterOfSquere = args[3];
    console.log(ifCircleInscribed(lengthOfCircle, perimeterOfSquere));
